"""
Класс cleaner для универсального KIR-анализатора.
Добавлена функция pre_clean() и универсальная поддержка group_cols.
"""
...
def pre_clean(input_file, output_file, target_col, remove_zeros=True, remove_empty_cols=None):
    """
    Предварительная очистка: удаление нулей и пустых значений.
    
    input_file: входной файл
    output_file: выходной файл
    target_col: целевой столбец (не удаляем строки с NaN в нем)
    remove_zeros: удалять ли строки с нулями в числовых столбцах
    remove_empty_cols: список столбцов для проверки на NaN
    """
    print(f"Предварительная очистка: {input_file}...")
    ...
    if target_col in df.columns:
        ...
        print(f"- Удалено строк с NaN в '{target_col}': {original_rows - len(df)}")
    ...
    if remove_zeros:
        ...
        for col in numeric_cols:
            ...
            if zeros_count > 0:
                df = df[df[col] != 0]
                print(f"- Удалено строк с нулями в '{col}': {zeros_count}")
    
    # Удаление по пустым столбцам
    if remove_empty_cols:
        for col in remove_empty_cols:
            if col in df.columns:
                empty_count = df[col].isna().sum()
                if empty_count > 0:
                    df = df[df[col].notna()]
                    print(f"- Удалено строк с NaN в '{col}': {empty_count}")
    
    new_rows = len(df)
    print(f"- Исходно: {original_rows}, после очистки: {new_rows} (удалено: {original_rows - new_rows})")
...
def split_outliers_multi(input_file, columns, group_cols=['НеделяГод', 'ТС', 'Категория'],
    ...
    columns: список столбцов для проверки, например ['КИР-ХХХ', 'Списания']
    group_cols: список колонок для группировки (может быть с категорией или без)
...
if __name__ == "__main__":
    ...
    COLUMNS_TO_CLEAN = ['Выручка', 'Списания', 'Свободный ТЗ']
